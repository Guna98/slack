export class MailinatorLoginBU {
    constructor() {
        this.elements = {
            inboxField: '#inbox_field',
            primaryBtn: 'button.primary-btn',
            //emailSubject: 'table[class="table-striped jambo_table"]',
    ...
    visitMailinator(mailinatorUrl, email) {
        ...
        cy.get(this.elements.inboxField).clear().type('{enter}'); // Clear inbox and press Enter
        cy.get(this.elements.inboxField).type(email); // Enter original email
    ...
    extractPassword() {
        cy.wait(5000); // Consider reducing this wait time if possible
                    ...
                    if ($body.find(elements.preferencesIcon).length > 0) {
                        cy.get(elements.preferencesIcon)
                            ...
                            .then(() => {
                                ...
                                cy.contains(elements.personalProfileButton, { timeout: 10000 })
                                    .should('be.visible')
                                    .then($btn => {
                                        if ($btn.length > 0) {
    ...
    updatePassword(baseUrl, oldPassword, index) {
        const elements = this.elements;
        cy.fixture('business_unit.json').then((collabData) => {
            const newPassword = collabData.password[index]; // Use index to get the correct password
            cy.origin(baseUrl, { args: { oldPassword, newPassword, elements } }, ({ oldPassword, newPassword, elements }) => {
                ...
                cy.get(elements.oldPasswordInput).clear().type(oldPassword);
                cy.get(elements.verifyButton).click().then(() => {
                    cy.contains('Please enter valid password').should('not.exist').then(() => {
                        cy.get(elements.newPasswordInput).clear().type(newPassword);
